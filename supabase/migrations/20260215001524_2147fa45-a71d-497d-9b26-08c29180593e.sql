...
-- Milestone 5 Patch: True atomic RPC
-- Fixes: EXCEPTION block now RAISE instead of RETURN, ensuring full rollback on failure.
-- Audit_logs is critical (failure = rollback). org_events is best-effort.
-- Stale check uses COALESCE(updated_at, created_at).
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- Reserved: check staleness using updated_at
    IF _existing.status = 'reserved' THEN
      IF COALESCE(_existing.updated_at, _existing.created_at) > (now() - _stale_threshold) THEN
  ...
  ELSE
    ...
    INSERT INTO executed_drafts (draft_id, workspace_id, agent_type, draft_type, executed_by, status, request_id, updated_at)
    VALUES (_draft_id, _workspace_id, _agent_type, _draft_type, _user_id, 'reserved', _request_id::text, now());
  END IF;

  -- ── 2. Agent writes (NO exception catching — failures propagate and rollback entire txn) ──
  IF _draft_type = 'draft_task_set' THEN
    _tasks := COALESCE(_payload->'tasks', '[]'::jsonb);
    
    FOR _task IN SELECT value FROM jsonb_array_elements(_tasks) AS t(value)
    LOOP
      INSERT INTO tasks (
  ...
  ELSE
    -- Unsupported draft type: RAISE to rollback the reservation
    RAISE EXCEPTION 'Unsupported draft_type: %', _draft_type;
  END IF;

  -- ── 3. Audit log (CRITICAL — failure rolls back everything) ──
  ...
  -- ── 4. Org event (best-effort — failure does NOT rollback) ──
  BEGIN
