...
BEGIN
  ...
  IF _hits > 2 THEN
    ...
    _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'FREQUENT_LIMIT_HITS', 'hits', _hits));
  END IF;

  -- Rule 2: projected breach → upgrade (only if limit is NOT NULL)
  IF _recommended != 'upgrade' THEN
    IF _plan_bookings_limit IS NOT NULL AND _proj_bookings >= _plan_bookings_limit THEN
      _recommended := 'upgrade';
      _confidence := GREATEST(_confidence, 75);
      _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'PROJECTED_BREACH', 'metric', 'bookings', 'projected', _proj_bookings, 'limit', _plan_bookings_limit));
    END IF;
    IF _plan_quotes_limit IS NOT NULL AND _proj_quotes >= _plan_quotes_limit THEN
      _recommended := 'upgrade';
      _confidence := GREATEST(_confidence, 75);
      _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'PROJECTED_BREACH', 'metric', 'quotes', 'projected', _proj_quotes, 'limit', _plan_quotes_limit));
    END IF;
  END IF;

  -- Rule 3: any metric > 80% → optimize (ONLY if that metric has a limit)
  IF _recommended = 'none' THEN
    IF (_plan_vendors_limit IS NOT NULL AND (_util->>'vendors')::int >= 80) THEN
      _recommended := 'optimize';
      _confidence := 55;
      _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'HIGH_UTILIZATION', 'metric', 'vendors', 'percent', (_util->>'vendors')::int));
    END IF;
    IF (_plan_services_limit IS NOT NULL AND (_util->>'services')::int >= 80) THEN
      _recommended := 'optimize';
      _confidence := 55;
      _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'HIGH_UTILIZATION', 'metric', 'services', 'percent', (_util->>'services')::int));
    END IF;
    IF (_plan_bookings_limit IS NOT NULL AND (_util->>'bookings')::int >= 80) THEN
      _recommended := 'optimize';
      _confidence := 55;
      _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'HIGH_UTILIZATION', 'metric', 'bookings', 'percent', (_util->>'bookings')::int));
    END IF;
    IF (_plan_quotes_limit IS NOT NULL AND (_util->>'quotes')::int >= 80) THEN
      _recommended := 'optimize';
      _confidence := 55;
      _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'HIGH_UTILIZATION', 'metric', 'quotes', 'percent', (_util->>'quotes')::int));
    END IF;
    IF (_plan_seats_limit IS NOT NULL AND (_util->>'seats')::int >= 80) THEN
      _recommended := 'optimize';
      _confidence := 55;
      _reasons := _reasons || jsonb_build_array(jsonb_build_object('code', 'HIGH_UTILIZATION', 'metric', 'seats', 'percent', (_util->>'seats')::int));
    END IF;
  END IF;

  -- Dynamic healthy confidence based on activity level
...
-- Ensure security: revoke public, grant authenticated
REVOKE ALL ON FUNCTION public.get_workspace_growth_insights(uuid) FROM PUBLIC;
