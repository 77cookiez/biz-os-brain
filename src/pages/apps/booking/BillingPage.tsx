...
export default function BillingPage() {
  ...
  const { plans, subscription, currentPlan, invoices, isLoading, invoicesLoading, changePlan } = useBilling();
  ...
  return (
    <div className="space-y-6 max-w-4xl">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg">
                  {currentPlan?.name || 'Free'} {t('billing.plan', 'Plan')}
                </CardTitle>
                <CardDescription>
                  {subscriptionMode === 'offline_invoice'
                    ? t('billing.offlineInvoice', 'Offline Invoice')
                    : t('billing.stripeManaged', 'Stripe Managed')}
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="space-y-4">
          {currentPlan && currentPlan.price_monthly > 0 && (
            <p className="text-2xl font-bold text-foreground">
              {formatCurrency(currentPlan.price_monthly, currentPlan.currency)}<span className="text-sm font-normal text-muted-foreground">/{t('billing.month', 'mo')}</span>
            </p>
          )}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <UsageBar
            label={t('billing.vendors', 'Vendors')}
            icon={<Users className="h-4 w-4" />}
            current={usage?.vendors_count ?? 0}
            limit={limits.vendors ?? null}
          />
          <UsageBar
            label={t('billing.bookingsMonth', 'Bookings (this month)')}
            icon={<CalendarCheck className="h-4 w-4" />}
            current={usage?.bookings_this_month ?? 0}
            limit={limits.bookings ?? null}
          />
          <UsageBar
            label={t('billing.services', 'Services')}
            icon={<Briefcase className="h-4 w-4" />}
            current={usage?.services_count ?? 0}
            limit={limits.services ?? null}
          />
          <UsageBar
            label={t('billing.quotesMonth', 'Quotes (this month)')}
            icon={<FileText className="h-4 w-4" />}
            current={usage?.quotes_this_month ?? 0}
            limit={limits.quotes ?? null}
          />
        </CardContent>
      </Card>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (subscriptionMode === 'offline_invoice') {
                          toast.info(t('billing.upgradeRequest', 'Upgrade request sent. Our team will contact you with an invoice.'));
                        }
        ...
        <CardContent className="py-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              {t('billing.enterpriseContact', 'Need a custom plan or have questions?')}
            </p>
